...
export default function ProductList() {
    ...
    // cada vez que `itemCount` cambie
    useEffect(() => {
        if (itemCount > 0) {
            setPopBadge(true);
            const timer = setTimeout(() => setPopBadge(false), 300); // duración de la animación
    ...
    const total = useMemo(() => {
        return cart.reduce((acc, item) => {
            const itemPrice = item.unitType === 'grams'
                ? (Number(item.price) * (item.quantity / 500))
        ...
        <div className="w-full max-w-2xl mx-auto p-4 md:p-6 bg-gray-50 min-h-screen">
            ...
            <div className="flex flex-col items-center mb-6">
                <Image
                    ...
                    width={120}
                    height={120}
                    className="mb-2 sm:w-28 sm:h-28 w-24 h-24" // tamaño más pequeño en móvil
                />
            ...
            {cart.length > 0 && (
                <>
                    ...
                    <div className="hidden sm:flex fixed bottom-6 right-6">
                        <Link
                            href="/carrito"
                            className="relative bg-amber-500 text-white font-bold py-3 px-6 rounded-lg shadow-lg hover:bg-amber-600 transition-transform transform hover:scale-105"
                        >
                            Ver Carrito (${cart.reduce((acc, item) => acc + Number(item.price) * item.quantity, 0).toFixed(2)})
                            {itemCount > 0 && (
                                <span
              ...
              transform transition-transform duration-200 ${popBadge ? "scale-110 rotate-3" : "scale-100 rotate-0"}`}
                                >
                                    {itemCount}
                                </span>
                            )}
